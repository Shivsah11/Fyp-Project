...
const LandlordDashboard = () => {
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (showRangeDropdown && !target.closest('.analytics-range-selector')) {
        setShowRangeDropdown(false);
      }
    };
        ...
        <div className="max-w-7xl mx-auto w-full pb-12">
          ...
          {activeSection === 'dashboard' ? (
            <div className="px-6 pt-6 mt-4">
              ...
              <div className={`flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8 p-6 rounded-2xl border backdrop-blur-xl ${isDarkMode ? 'bg-gray-800/50 border-gray-700/50' : 'bg-white/80 border-gray-200'} shadow-sm`}>
                <div>
                  <h3 className={`text-xl font-bold tracking-tight ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>Dashboard Overview</h3>
                  <p className={`text-sm mt-1 ${isDarkMode ? 'text-gray-400' : 'text-gray-500'}`}>Here's a quick summary of your properties.</p>
                </div>
                <div className="flex flex-wrap gap-3">
                  <button
                    onClick={() => document.getElementById('analytics-section')?.scrollIntoView({ behavior: 'smooth' })}
                    className="flex items-center gap-2 bg-gradient-to-r from-blue-500/10 to-indigo-500/10 hover:from-blue-500 hover:to-indigo-600 hover:text-white text-blue-600 dark:text-blue-400 px-5 py-2.5 rounded-xl font-medium transition-all duration-300 shadow-sm border border-blue-200 dark:border-blue-800/50 group"
                  >
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white px-5 py-2.5 rounded-xl font-medium transition-all duration-300 shadow-md transform hover:-translate-y-1 hover:shadow-lg border border-blue-500/50"
                  >
              ...
              <div id="analytics-section" className="mt-8 pt-8 border-t border-gray-200/50 dark:border-gray-700/50">
                ...
                <div className="flex items-center justify-between mb-6">
                  ...
                  <div className="relative analytics-range-selector">
                    ...
                    {/* Range Dropdown Menu */}
                    {showRangeDropdown && (
                      <div className={`absolute right-0 mt-3 w-48 rounded-2xl border backdrop-blur-xl shadow-2xl z-50 overflow-hidden transform origin-top transition-all duration-300 animate-in fade-in zoom-in-95 ${
                        isDarkMode ? 'bg-gray-900/90 border-gray-700/50' : 'bg-white/90 border-gray-100'
                      }`}>
                        <div className="p-2 space-y-1">
                          {rangeOptions.map((option) => (
                            <button
                              ...
                              className={`w-full text-left px-4 py-2.5 rounded-xl text-sm font-black italic transition-all duration-200 ${
                                analyticsRange === option.id
                ...
                {/* Revenue Chart */}
                <div className={`rounded-xl border p-6 mb-6 transition-all duration-300 hover:shadow-md ${isDarkMode ? 'bg-gray-800/80 border-gray-700' : 'bg-white/90 border-gray-200'}`}>
                  <h4 className={`font-bold text-lg mb-6 ${isDarkMode ? 'text-gray-100' : 'text-gray-800'}`}>Monthly Revenue Trend</h4>
                  <div className="space-y-4">
                    {analytics.monthlyRevenue.map((revenue, index) => (
                      <div key={index} className="flex items-center gap-4 group">
                        <span className={`text-sm font-medium w-20 ${isDarkMode ? 'text-gray-400 group-hover:text-gray-200' : 'text-gray-600 group-hover:text-gray-900'} transition-colors`}>Month {index + 1}</span>
                        <div className={`flex-1 rounded-full h-8 overflow-hidden border ${isDarkMode ? 'bg-gray-800/50 border-gray-700' : 'bg-gray-100/50 border-gray-200'}`}>
                          <div
                            className="bg-gradient-to-r from-blue-500 to-indigo-600 h-full rounded-full flex items-center justify-end pr-4 transition-all duration-1000 ease-out shadow-inner"
                            style={{ width: `${(revenue / Math.max(...analytics.monthlyRevenue)) * 100}%` }}
                          >
                            <span className="text-white text-sm font-bold tracking-wide">NPR {revenue.toLocaleString()}</span>
                          </div>
                        </div>
                      </div>
                    ))}
                ...
                <div className={`rounded-xl border p-6 mb-6 ${isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200'}`}>
                  ...
                  <form onSubmit={handleAddProperty} className="grid grid-cols-2 gap-4">
                    <input
                      ...
                      className={`px-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 ${isDarkMode
                    ...
                    <div className="relative">
                      <input
                        ...
                        className={`w-full px-4 py-3 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 pr-24 ${isDarkMode
                          ? 'bg-gray-700 border-gray-600 text-white placeholder-gray-400 hover:bg-gray-600'
                          : 'bg-white border-gray-300 text-gray-700 placeholder-gray-400 hover:bg-gray-50'
                          }`}
                        required
                    ...
                    <div className="col-span-2">
                      ...
                      <div className="mb-3">
                        ...
                        <label
                          ...
                          className={`w-full px-4 py-3 rounded-xl cursor-pointer text-center focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all duration-300 ${isDarkMode
